  ...
  for (i in seq_along(model_names)) {
    ...
    for (j in 1:4) {
      ...
      model_file_name <- list.files(
        paste0(
          "stock-specific/", spp, "/output/density-models/",
      ...
      if (file.exists(full_model_path)) {
        filename <- paste0(
          "stock-specific/", spp, "/output/density-effects/dens-",
...
"
  ...
  ggsave(paste0("stock-specific/", spp, "/figs/dens-effects-", variable, ".png"),
    height = if (length(model_names) > 3) {
      round(length(model_names) / 2 + 1) * 1.5
    } else {
      4
    },
