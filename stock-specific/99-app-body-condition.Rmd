...
dens_model_name0 <- ####
model_date <- ####
knot_distance <- "20" # current default for coastwide stocks
```
...
mc_data <- bind_rows(mc$data, mc2$data, mc3$data)
rm(mc,mc2,mc3)
...
msa_min <- min(mc_data[mc_data$survey_group == "MSA",]$year)
hake_years <- length(unique(mc_data[mc_data$survey_abbrev %in% c("HAKE"),]$year))
```
...
an integrated acoustic-trawl surveys (HAKE) targeting Pacific Hake (*Merluccius productus*), 
and two more targeted historic trawl surveys--the west coast Vancouver Island thornyhead and Hecate Strait Pacific Cod surveys.
...
The Hecate Strait Pacific Cod (*Gadus macrocephalus*) survey (HS PCOD) targeted known hotspots for this species annually for three years [2002-2004, @sinclair2002]. 
The depths sampled ranged from 22 to 168~m. 
Abundance data was collected for all groundfish species encountered, but morphological data was collected only for a couple target species. 
...
The integrated acoustic-trawl surveys (HAKE) targeting Pacific Hake (*Merluccius productus*) have been conducted in Canadian waters since 1977 [@deblois2020}.
...
Morphological data was collected for `r species` in only `r hake_years` years (`r unique(mc_data[mc_data$survey_abbrev %in% c("HAKE"),]$year)`).


<!-- Include only relevant details not provided in other appendices -->
Additional morphological data was also collected by longline hook and trap surveys.
These surveys were not used for calculating biomass density because the catches were recorded in numbers rather than weight and area swept estimates are harder to generate.
However, biological sample data from these surveys were used in estimation of length-weight relationships, length at maturity, and body condition.

The Outside Hard Bottom Longline surveys (HBLL) are conducted biennially beginning in 2006 for the northern region and 2007 for the southern region [@anderson2019synopsis; @doherty2019hbllout]. 
These surveys targeted hard-bottom habitats in untrawlable areas and are of a stratified random design with 3 depth strata: shallow (20-70 m), medium (71-150 m) and deep (151-260 m).
Rockfish maturity and length data have been collected from the start of these surveys, with weights recorded starting in 2009. 

The International Pacific Halibut Commission (IPHC) fishery-independent longline survey has a fixed station design with stations located at the intersections of an 18.52~km (10~nmi) square grid intended to span depths ranging from 18--732~m [@IPHCmanual].
At a randomly selected subset of these stations, eight skates of 100~($\pm5\%$) size 16/0~hooks with 18~ft (5.49~m) spacing between each hook were set between May 24th and August 26th.
Maturity and length data was collected for most rockfish species since 2003, with weights starting in 2009. 

The offshore trap surveys targetting sablefish (*Anoplopoma fimbria*) have evolved from fixed stations (1988 - 2010) to a stratified random design [2003-2023, @lacko2023].
The gear used consists of a groundline resting on the ocean floor with 25 baited traps attached at 150 foot intervals and a targeted soak time of 24 hours.
...
<!-- Add explanation for when dens_model_name does not include "only-sampled" -->
For unsampled tows in any survey-year combinations with less than 6 tows, we applied the average proportion of each class from all surveys in that year (as long as the average SD in proportions across all years within that survey was greater than the SD across all surveys within each year, and at least six tows were measured in all surveys that year), or for all years for each survey (if there were fewer than six tows across all surveys with measurements in that year, or the average SD in proportions across all years within each survey was less than the SD across all surveys within each year). 
Finally, for surveys with fewer than this threshold over all years, the global average proportions were applied. 
...
<!-- Check that family is still correct here -->
The subscripts 1 and 2 denote the equivalent parameters from the first (Bernoulli) and second (`r if(dens_model_name0 == "dln-"){"lognormal"}else{"????"}`) linear predictors.
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/C-density-map-",spp,"-mat-fem-",dens_model_name,"-", knot_distance, "-km.png")))
```
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/C-density-map-",spp,"-mat-m-",dens_model_name,"-", knot_distance, "-km.png")))
```
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/C-density-map-",spp,"-imm-",dens_model_name,"-", knot_distance, "-km.png")))
```
