...
#                       fill_label = rosettafish::en2fr("Total biomass", FRENCH),
...
mat$maturity_plot +
  labs(
    ...
    y = rosettafish::en2fr("Probability mature", FRENCH),
    linetype = rosettafish::en2fr("Sex", FRENCH),
    colour = rosettafish::en2fr("Sex", FRENCH)
  ) + theme(
...
ggplot(
  dat |> filter(
    ...
    mutate(
      ...
      sex_label = rosettafish::en2fr(sex_label, FRENCH),
      weight = weight / 1000,
  ...
  labs(
    ...
    x = rosettafish::en2fr("Length (cm)", FRENCH),
    y = rosettafish::en2fr("Weight (kg)", FRENCH)
  ) +
...
(g <- d |> ggplot(aes(year, est/1000*4,
    ...
    geom_line(aes(colour = group, linetype = rosettafish::en2fr("Split by sex and maturity", FRENCH))) +
    geom_ribbon(aes(alpha = group, fill = group)) +
    geom_line(data = ds, colour = "black", aes(linetype = rosettafish::en2fr("Sum of split indices", FRENCH))) +
    geom_line(data = d0, colour = "black", aes(linetype = rosettafish::en2fr("Index of total biomass", FRENCH))) +
