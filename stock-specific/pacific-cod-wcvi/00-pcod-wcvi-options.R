...
survey_grids <- c("SYN WCVI")

## set surveys from which any data can be used (maturities, condition, and/or densities) ----
## use default survey_abbrev
## ok to include ones that are outside area of interest as they will be filtered out by area selection
...
## only converged for knot_distance of 5km
## all Q <= 0.2, so close enough to justify lognormal with much faster 10km?
...
knot_distance <<- 10 # for WCVI models
...
# TODO: revise to work better for local stocks
...
# should apply to density predictions as well
# for now using list of surveys and the survey grid
...
rm(m)
## or restart before running due to m object getting retained in global environ
...
### plot condition maps -- run last because slowest
